...
from SeiraRobot import TEMP_DOWNLOAD_DIRECTORY, pbot


@pbot.on_message(filters.command("anh18+"))
async def boobs(client, message):
    if not os.path.isdir(TEMP_DOWNLOAD_DIRECTORY):
        os.makedirs(TEMP_DOWNLOAD_DIRECTORY)
    pic_loc = os.path.join(TEMP_DOWNLOAD_DIRECTORY, "bobs.jpg")
    a = await message.reply_text("**Tìm kiếm hình ảnh khỏa thân**")
    await asyncio.sleep(0.5)
    await a.edit("`Gửi ảnh khỏa thân...`")
    nsfw = requests.get("http://api.oboobs.ru/noise/1").json()[0]["preview"]
    urllib.request.urlretrieve("http://media.oboobs.ru/{}".format(nsfw), pic_loc)
    await client.send_photo(message.chat.id, pic_loc, caption="**Nó tuyệt vời, đừng ngu ngốc**")
    os.remove(pic_loc)
    await a.delete()
